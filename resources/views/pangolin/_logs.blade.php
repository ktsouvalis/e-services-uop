<div class="bg-white shadow-sm sm:rounded-lg p-6 mb-6">
    ...
    <p class="text-sm text-gray-500 mb-4">{{ __('Pulls logs at or above the chosen level from every node over SSH and, if Newt hosts are configured, resolved access sessions.') }}</p>
    <form action="{{ route('pangolin.logs.fetch') }}" method="POST" class="flex items-end gap-4">
        @csrf
        <div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead>
                <tr class="text-left text-xs font-medium text-gray-500 uppercase">
                    ...
                    <th class="px-3 py-2">{{ __('Files') }}</th>
                </tr>
            </thead>
                ...
                @forelse ($logRuns as $run)
                    <tr>
                        ...
                        <td class="px-3 py-2 space-x-3">
                            @if ($run->report_path)
                                <a href="{{ route('pangolin.logs.download', $run) }}" class="text-indigo-600 hover:underline">{{ __('log') }}</a>
                            @endif
                            @if ($run->extra_path)
                                <a href="{{ route('pangolin.logs.download', ['run' => $run, 'file' => 'newt']) }}" class="text-indigo-600 hover:underline">{{ __('newt csv') }}</a>
                            @endif
                        </td>
                    </tr>
                @empty
